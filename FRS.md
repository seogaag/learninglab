...
### 3.2 고정 공지 (Pinned Notices)

| 항목 | 내용 |
|------|------|
| 요구사항 ID | HOME-002 |
| 설명 | 메인 페이지에 고정된 Community 공지 표시 |
| 데이터 소스 | `GET /public/pinned-notices` |
| 클릭 동작 | Community 게시글 상세로 이동 |
...
| 미로그인 | 로그인 안내 및 Sign In 버튼 |
| 권한 부족 | 권한 다시 부여(재로그인) 안내 |
| 클릭 동작 | 클래스 카드 클릭 시 아무 동작 없음 (VIEW COURSE 링크로 이동) |
...
| 필터 | 태그별 필터링, 검색(제목/내용) |
| 정렬 | 고정 게시글 우선, 최신순 |
| 상세 | 제목, 내용, 작성자, 작성일시, 조회수, 좋아요, 댓글 수 |
...
| 2.0.0 | 2025-02-23 | 현재 버전 반영, 기능별 표 형식, 우선순위 제거 |
